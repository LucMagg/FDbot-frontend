...
class Mercregister(commands.Cog):
  ...
  async def mercregister_app_command(self, interaction: discord.Interaction, héros: str, ascend: str|None = None, pet: str|None = None):
  ...
  async def get_response(self, user_name, user_id, hero, ascend, has_pet):
    ...
    user = await self.add_merc_to_user(user_name, user_id, hero, ascend, has_pet)
    if user:
  ...
  async def add_merc_to_user(self, user_name, user_id, hero, ascend, has_pet):
