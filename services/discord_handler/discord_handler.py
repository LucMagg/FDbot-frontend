...
class DiscordHandler:
  ...
  async def send(self, data: BaseUiData):
    self.logger.log_only('debug', f'[IH] Interaction {data.interaction.id} | User {data.interaction.user.id} [{nick(data.interaction)}]')
    payload = {}
    try:
      if data.modal:
        self.logger.log_only('debug', f'[IH] Sending modal for interaction {data.interaction.id}')
      ...
      self.logger.log_only('error', f'[IH] Interaction {data.interaction.id} | {e}')
  ...
  async def send_from_loop(self, data: LoopUi):
    self.logger.log_only('debug', f'[IH] Channel {data.channel.id}')
    ...
    try:
      ...
      self.logger.log_only('error', f'[IH] Channel {data.channel.id} | {e}')
      fallback_result = await self._hard_fallback(data)
      return fallback_result
  ...
  async def send_from_channel(self, data: BaseUiData):
    ...
    try:
      ...
      self.logger.log_only('debug', f'[IH] Result: {result}')
      if result:
  ...
  async def _send_modal(self, data: BaseUiData):
    try:
      ...
      self.logger.log_only('warning', f'[IH] Modal: interaction {data.interaction.id} already responded')
      if data.previous_interaction:
        try:
          return await data.previous_interaction.send_modal(data.modal)
        except Exception:
          self.logger.log_only('error', f'[IH] Modal fallback failed | interaction={data.interaction.id} | previous={data.previous_interaction.id}')
          return await data.previous_interaction.followup.send(embed=self.payload_builder.build_generic_error(data))
      else:
        self.logger.log_only('error', f'[IH] Modal failed, no previous interaction | {data.interaction.id}')
  ...
  async def _hard_fallback(self, data: BaseUiData):
    ...
    self.logger.log_only('error', '[IH] Hard fallback triggered')
    if data.previous_interaction:
      try:
    ...
    try:
      ...
      self.logger.log_only('error', f'[IH] Total failure: {e}')
      return None
