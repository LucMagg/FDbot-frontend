...
from utils.str_utils import str_to_slug, int_to_str, format_float
...
class LevelService:
  ...
  async def display_rewards(self, emojis, level_name):
    ...
    if len(level.get('rewards')) > 0:
      description += self.get_rewards_str(level, emojis)
    else:
      description += 'Aucune récompense recueillie pour l\'instant :shrug:\nN\'hésitez pas à en ajouter à l\'aide de la commande /reward :wink:'
  ...
  async def add_reward(self, emojis, level_name, reward_data: dict):
    ...
    if not level:
      return {'title': 'Erreur', 'description': 'Le bot a rencontré une erreur, merci de réessayer plus tard :shrug:', 'color': 'red'}
  ...
  def get_rewards_str(self, level, emojis) -> str:
    ...
    for r in level.get('rewards'):
      ...
      elif len(level.get('reward_choices')) > 1:
        to_return += self.append_with_multiple_reward_types(r, icon)
      else:
  ...
  def append_with_multiple_reward_types(self, reward, icon):
    to_return = f"\n{icon} {reward.get('quality')} {reward.get('type')} : {format_float(reward.get('total_appearances') / self.total_appearances * 100, 2)}% ({reward.get('total_appearances')}), soit :\n"
    for d in reward.get('details'):
      ...
      average = format_float(d.get('appearances') / reward.get('total_appearances') * 100, 2)
      to_return += f' : {average}% ({d.get('appearances')})\n'
    return to_return
  ...
  def append_with_single_reward_type(self, level, reward, icon):
    ...
    for d in reward.get('details'):
      ...
      average = format_float(d.get('appearances') / reward.get('total_appearances') * 100, 2)
      to_return += f'\n{icon} {quantity} {type} : {average}% ({d.get('appearances')})\n'
  ...
  def energy_stats(self, level, reward, icon):
    ...
    if average_reward < 1000:
      displayed_average_reward = format_float(average_reward, 3)
    else:
    ...
    try:
      ...
      for energy in to_check:
        ...
        if energy_cost is not None:
          ...
          if average_reward < 1000:
            displayed_average_per_energy = format_float(average_per_energy, 3)
          else:
